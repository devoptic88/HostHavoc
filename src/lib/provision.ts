...
import { cancelPaymenterOrder } from "@/lib/paymenter";
 ...
 * Best-effort: an already-cancelled order must not block termination.
 */
export async function terminateOrder(orderId: string): Promise<void> {
  ...
  if (order.paymenterOrderId) {
    await cancelPaymenterOrder(order.paymenterOrderId);
  }
